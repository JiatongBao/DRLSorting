...
# --------------- Setup options ---------------
tcp_host_ip = "192.168.8.113"    # IP and port to robot arm as TCP client (UR5)
tcp_port = 30002
rtc_host_ip = "192.168.8.113"    # IP and port to robot arm as real-time client (UR5)
rtc_port = 30003
# workspace_limits = np.asarray([[-0.16, 0.21], [-0.35, -0.25], [0.30, 0.36]]) # Cols: min max, Rows: x y z (define workspace limits in robot coordinates)
workspace_limits = np.asarray([[-0.10418, 0.17582], [-0.38669, -0.20669], [0.255, 0.355]])
calib_grid_step = 0.05
checkerboard_offset_from_tool = [-0.019, 0, -0.085]   # [-0.019, 0, -0.085]
tool_orientation = [2.2854, 2.0636, -0.1541]
# ---------------------------------------------
...
print('*' * 32)
robot = Robot(False, None, None, False, workspace_limits,
              tcp_host_ip, tcp_port, rtc_host_ip, rtc_port,
              False, None, None)
robot.open_gripper()

# Slow down robot
robot.joint_acc = 0.05
robot.joint_vel = 0.1

# Make robot gripper point upwards
deg2rad = np.pi / 180
robot.move_joints((41.02 * deg2rad, -42.77 * deg2rad, 27.91 * deg2rad, -75.34 * deg2rad, -89.73 * deg2rad, 16.45 * deg2rad))
...
for calib_pt_idx in range(num_calib_grid_pts):
    ...
    if checkerboard_found:
        print('Checkerboard has been found!!!')
